...
import { generateMgmServerActivityRankingPdf } from "../pdf/generateMgmServerActivityRankingPdf";

export default function MgmServerActivityRanking() {
...
mgmData.forEach((entry) => {
  ...
  // Count unique alliances on this server
  grouped[server].alliances.add(entry.id);

  // Count MGM entries
  grouped[server].mgms += 1;
  ...
  // Oldest MGM date for tie-breaking
  if (
    new Date(entry.date) <
    new Date(grouped[server].oldestDate)
  ) {
    grouped[server].oldestDate = entry.date;
  }
});
  ...
  .map((entry) => ({
    ...entry,
    alliances: entry.alliances.size,
  }))
      .sort((a, b) => {
    ...
    <>
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-16">
        ...
        <p className="mt-3 text-slate-400">
          Rank servers by their total participant activity across
        ...
        <div className="mt-10">
          ...
          <div className="mt-4 flex flex-col gap-4 lg:flex-row">
            ...
            <button
              onClick={() =>
                generateMgmServerActivityRankingPdf(
                  filteredRanking,
                  dataset
                )
              }
